...
def addMealToCollection(userID,collectionName,mealID):
    ...
    try:
        ...
        db.session.rollback()
        logging.exception("Failed to add meal %s to collection %s", mealID, collectionName)
        raise
    return True
...
def createNewCollection(userID,collectionName):
    ...
    try:
        ...
        db.session.rollback()
        logging.exception("Failed to create collection %s for user %s", collectionName, userID)
        raise
    return True
...
def saveNewMeals(userID, newMeals, units=None, quants=None):
    ...
    if saved_any:
        try:
            ...
            db.session.rollback()
            logging.exception("Failed to save new meals for user %s", userID)
            raise
    return saved_any
